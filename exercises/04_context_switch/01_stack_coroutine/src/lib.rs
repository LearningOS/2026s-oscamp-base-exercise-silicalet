...
impl TaskContext {
    ...
    pub fn init(&mut self, stack_top: usize, entry: usize) {
        ...
        // Ensure stack_top is 16-byte aligned (RISC-V ABI requirement)
...
pub unsafe fn switch_context(old: &mut TaskContext, new: &TaskContext) {
    ...
    core::arch::naked_asm!(
        ...
        "ld s10, 96(a1)",
        "ld s11, 104(a1)",
        // optional: clear argument registers so old/new pointers
        // are not leaked into the resumed context
        "li a0, 0",
        "li a1, 0",
        // jump to new.ra
        "ret",
...
pub fn alloc_stack() -> (Vec<u8>, usize) {
    ...
    // Ensure 16-byte alignment for RISC-V ABI
